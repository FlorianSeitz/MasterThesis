...
\end{equation}
where $d_{ij}$ is the Minkowski distance between probe $i$ and exemplar $j$, $x_{im}$ is the value of probe $i$ on feature $m$, $w_{m}$ is the attention weight attributed to dimension $m$ (with $0 \leq w_{m} \leq 1$ and $\sum w_{m} = 1$), $M$ is the number of dimensions, and $r$ describes the form of the distance metric. In dependence of the  value of $r$, the Minkowski distance can take countless forms such as the Manhattan distance ($r = 1$) or the Euclidean distance ($r = 2$, a generalized form of the Pythagorean theorem for $n$ dimensions). With higher values of $r$ absolute differences on dimensions between the probe and exemplar are increasingly exponentially scaled. This means that as the absolute differences on a dimension increases, its share of the total distance increases faster with higher values of $r$. Values of $r$ are typically constrained to be equal to or higher than 1, as for $0<r<1$ the triangle inequality, an axiom needed for a distance of the Generalized Context Model to be a metric \textbf{explain more precisely}, is violated \citep{jakel2008similarity,francois2007concentration,tversky1982similarity,beals1968foundations}. In such a case, the distance is not anymore a metric \citep[][p. 5]{kress1989linear}. 

%To Do: Write that Minkowski is geometric (cite Goldstone for other similarity calculations) and then write below that geometric models with r < 1 don't fulfill the triangle inequality axiom. Highly separable vs. integral

The distance $d_{ij}$ is then transformed into a measure of similarity by means of an exponential decay function \citep{nosofsky1986attention}:
\begin{equation}
s_{ij} = \exp\left(-c*d_{ij}^p\right),
\end{equation}
where $s_{ij}$ is the similarity between exemplars $i$ and $j$, $c$ (with $0 \leq c$) is an overall sensitivity parameter, and $p$ .

\begin{equation}
P(R_{A}|S_{i}) = \frac{b_{A}\sum\limits_{j \in C_{A}} s_{ij}}{b_{A}\sum\limits_{j \in C_{A}} s_{ij} + (1 - b_{A})\sum\limits_{k \in C_{B}} s_{ik}}
\end{equation}
