...
Participants selected themselves into the sample and there were no inclusion criteria. Up to six participants could show up to the same experimental session. %
...
\end{tabular}
\begin{tablenotes}[para]
\textit{Note.} Fixed effects estimates for the linear mixed model with logit link. Coefficients were calculated using sum--to--zero contrasts \textit{cite}. Baseline levels are the no time pressure condition and the aggregated stimuli 221, 231, 321, and 331.
\end{tablenotes}
\end{threeparttable}
\end{center}
\end{sidewaystable}
